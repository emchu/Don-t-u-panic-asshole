...
class Controls:
    ...
    def draw(self):
        ...
        self.title.render_to(self.game.screen, (100, self.size[1] / 8), "Sterowanie", black)

        # WSAD

        self.text.render_to(self.game.screen, (self.size[0] / 4 - 65, self.size[1] / 3 + 160), "Poruszanie", black)
        pygame.draw.rect(self.game.screen, black, (self.size[0] / 4 - 80, self.size[1] / 3, 230, 150), 2)

        # Equipment I

        self.text.render_to(self.game.screen, (self.size[0] / 2 - 100, self.size[1] / 3 + 160), "Ekwipunek", black)
        pygame.draw.rect(self.game.screen, black, (self.size[0] / 2 - 40, self.size[1] / 3 + 70, 70, 70), 2)

        # Use E

        self.text.render_to(self.game.screen, (self.size[0] * (3 / 4) - 170, self.size[1] / 3 + 160), "Użyj", black)
        ...
        self.text.render_to(self.game.screen, (self.size[0] / 4 - 90, self.size[1] * (3/4) + 80), "Szybki dostęp", black)
        pygame.draw.rect(self.game.screen, black, (self.size[0] / 4 - 160, self.size[1] * (3/4), 390, 70), 2)

        # LPM + mouse

        self.text.render_to(self.game.screen, (self.size[0] / 2 + 30, self.size[1] * (3/4) + 80), "Atak", black)
        pygame.draw.rect(self.game.screen, black, (self.size[0] / 2, self.size[1] * (3/4) - 70, 140, 140), 2)

        self.text.render_to(self.game.screen, (self.size[0] * (3 / 4) - 100, self.size[1] * (3 / 4) + 80), "Celowanie", black)
        ...
        if pos_x + 100 > mouse[0] > pos_x and pos_y + 40 > mouse[1] > pos_y:
            self.menu.render_to(self.game.screen, (pos_x, pos_y), "Wróć" , green)
            pygame.draw.rect(self.game.screen, green, (pos_x, pos_y, 100, 40), 2)
            if click[0]:
                self.game.current_screen = self.exit_screen
        else:
            self.menu.render_to(self.game.screen, (pos_x, pos_y), "Wróć", black)
